...
import ODataV4ListBinding, {
  ODataListBinding$CreateCompletedEvent,
} from "sap/ui/model/odata/v4/ODataListBinding";
...
const oFileUploader = new FileUploader({
  ...
  uploadComplete: (oEvent: FileUploader$UploadCompleteEvent) => {
    // 1.115.1: types not only for event parameters, but also for events
...
dateTimePicker.setShowCurrentTimeButton(
  !dateTimePicker.getShowCurrentTimeButton()
);
...
const odataV4ListBinding = illustratedMessage.getBinding(
  "additionalContent"
...
sap.ui.require(
  ["sap/m/Button", "sap/m/Input"],
  (B: typeof Button, I: typeof Input) => {
...
odataV4ListBinding.attachCreateCompleted(
  (evt: ODataListBinding$CreateCompletedEvent) => {
