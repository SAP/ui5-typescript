 ...
 */
export type PropertyByAbsoluteBindingPath<Type, Path extends string> = Path extends `/${number}`
...
type FromArrayWithSubPath<Type, Key extends keyof Type, Rest extends string> =
  Type[Key] extends Array<infer U> ? PropertyByAbsoluteBindingPath<U, `/${Rest}`> : never;
 ...
 */
type FromNestedProperty<Type, Key extends keyof Type, Rest extends string> = PropertyByAbsoluteBindingPath<
  Type[Key],
  `/${Rest}`
>;
 ...
 */
type TypeAtPath<Type, Path extends string> = Path extends `/${infer Key}/${infer Rest}`
  ? Key extends keyof Type
