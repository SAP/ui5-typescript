...
How can apps now use a different version of the jQuery, qUnit,... types?
  ...
  * As soon as there is an own copy of the jquery types below "node_modules/@openui5/types", nothing will move it back up to the main `node_modules/@types` directory. Not even requesting the exactly same version and deleting `package-lock.json` and deleting the toplevel copy of the jquery types. Only when the copy of the types below "node_modules/@openui5/types" is deleted and another `npm i` is done, npm is happy with only placing ONE copy of the jquery types to the main node_modules folder again (if versions match).
